...
if __name__ == '__main__':
    ...
    female_name_file = 'female_firstnames.txt'
    female_word_file = 'female_words.txt'
    auc_filename = r'auc.png'
    ...
    if not any([options.run_all, options.run_data, options.run_features, 
                options.run_prediction]):
    ...
    if options.run_all or options.run_features:
        data = generate_feature_csv(csv_in=full_csv_filename, 
                                    csv_out=features_csv_filename,
                                    female_name_file=female_name_file,
                                    female_word_file=female_word_file,
                                    verbose=options.verbose)
    
    if options.run_all or options.run_prediction:
        bechdel_prediction(features_csv=features_csv_filename,
                           auc_filename=auc_filename,
                           bootstrap_runs=100,
                           verbose=options.verbose)
